...
if (isset($config['sms']['enabled']) && $config['sms']['enabled'] === true && isset($sms_type))
{
	...
	elseif (isset($phone))
	{
		if ($sms_type == 'order2customer' || $sms_type == 'paid2customer' || $sms_type == 'status2customer' || $sms_type == 'call2customer' || $sms_type == 'trackSent2customer' || $sms_type == 'trackDelivered2customer' || $sms_type == 'sdekSent2customer')
			$sms_reseiver = $phone;
		else
			$sms_reseiver = false;
	}
	...
	$sum = (isset($paid_sum)) ? $paid_sum : ((isset($order_sum)) ? $order_sum : $order['sum']);
	
	# Выборка трека
	if (!isset($track_id) && isset($order['track']))
	...
	if (isset($track_place))
		$sms_text = str_replace('{{track_place}}', $track_place, $sms_text);
