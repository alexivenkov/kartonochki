	...
	$.fn.countdown = function (date, options) {
		...
		var timeDifference = function(begin, end) {
		    ...
		    for (i in diff) {
			    ...
			    result.push(((((diff[i][0]+'').length)==1)?'0'+diff[i][0]:((((diff[i][0]+'').length)==0)?'00':diff[i][0])) + ' ' + options.lang[i][options.lang.plurar(diff[i][0])]);
		    }
		...
		var timeUpdate = function () {
		    ...
		    if (s.length) {
				...
				setTimeout(function () {
					...
					if (options.replace)
						window.location.replace(options.replace);
				}, options.timeout);
		    }
		};
		timeUpdate();
